...
const NAME_MESSAGE = `Blocked: pkill / killall kill by name, and processes you did not start share those names.
This machine runs several agents and the operator's own dev servers, so a Vite or Playwright server you meant to stop looks the same as theirs. On 2026-08-18, in the DorkOS repo, one \`pkill -f\` meant for a single test server also killed the operator's dev server.
