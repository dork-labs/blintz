...
## Blintz-Specific Patterns

- **Location**: unit tests sit next to the source they cover, as `*.test.ts` or `*.test.tsx` under `packages/*/src/`. Nothing outside that glob runs (root `vitest.config.ts`).
- **Environment**: the default is `node`. A test that mounts the editor needs a DOM; copy the setup from `packages/blintz/src/reactive-editable.test.tsx`.
- **Round-trip first**: for a parsing or serialization change, the failing test is a markdown string that does not come back unchanged. Add it to `round-trip.test.ts`.
- **Visual behavior** (typography, themes, spacing) is tested in `browser-tests/` with Playwright screenshots, not in jsdom. Watch the screenshot comparison fail before changing CSS.

### Commands

```bash
npx vitest run path/to/file.test.ts   # one file — the TDD inner loop
npm test                              # full unit suite
npm run test:browser                  # Playwright, when rendering changed
```
