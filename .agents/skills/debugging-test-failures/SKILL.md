...
3. **Check load-sensitivity before diving in**
   ...
   - the failure text names milliseconds, wall-clock boundaries, or "expected
     N samples"
   - it is a Playwright spec in `browser-tests/` that timed out rather than
     failed an assertion, or a screenshot diff on a machine that is not macOS
     (the baselines are `*-darwin.png`)
