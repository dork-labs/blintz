...
/**
 * Ported verbatim from Crepe (`crepe/src/utils/group-builder.ts`) — pure TS, no
 * framework. Builds grouped menu/toolbar items; the slash menu additionally
 * relies on the flat `index` + per-group `range` it can assign. Reused as-is by
 * the toolbar (P0) and, later, the slash menu (P1).
 */
type MenuItem<T> = {
  index: number;
  key: string;
...
type WithRange<
  T,
  HasIndex extends true | false = true,
> = HasIndex extends true ? T & { range: [start: number, end: number] } : T;
